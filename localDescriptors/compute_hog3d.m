function [ features ] = compute_hog3d( imgs,index,indexValue, orientation, para, fType )
%COMPUTE_HOG3D Summary of this function goes here
...
% 					part_zindex = zstart:zend;
					part_zindex = zstart:zend-1;
				end
				imgSeqPart = imgSeq(part_yindex,part_xindex,c,part_zindex);
				descTemp = hog3d_block(imgSeqPart,6);	% 1*48
				descPart = [descPart; descTemp(:)];		% 48*2*9 = 864
			end
		end
		desc = [desc; descPart(:)];	% 864*3 = 2592
	end	% for c
