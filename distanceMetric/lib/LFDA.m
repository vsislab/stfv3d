...
% belta = 0.5;
% tSw = (1-belta)*tSw + belta*trace(tSw)/n*eye(size(tSw));
...
% tSb=tSb+eye(size(tSb))*min(tSb(:))/100;
% tSw=tSw+eye(size(tSw))*min(tSw(:))/100;
...
%   [eigvec,eigval_matrix]=eigs(tSb,tSw,r,'la',opts);
  [eigvec,eigval_matrix]=eigs(tSb,tSw,r);
end
